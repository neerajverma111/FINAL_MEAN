...
export function MyBookings() {
  ...
  async function deleteBooking(bookingid, roomid) {
    try {
      setLoading(true)
      const result = await (await axios.post("/api/bookings/deletebooking", { bookingid, roomid })).data
      console.log(result)
      setLoading(false)
    } catch (error) {
      console.log(error)
                ...
                return(
                  <div className='bs'>
                    ...
                    {/* <div className='text-right'>
                      <button class='btn btn-primary' onClick={() => { deleteBooking(booking._id, booking.roomid) }}>DELETE BOOKING</button>
